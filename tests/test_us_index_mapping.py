...
# 确保项目根目录在 sys.path 中，以便正确导入 data_provider 包
...
class TestIsUsIndexCode(unittest.TestCase):
    ...
    def test_known_indices(self):
        ...
        for code in indices:
            with self.subTest(code=code):
                self.assertTrue(
                    is_us_index_code(code), f"{code} should be recognized as US index"
                )
    ...
    def test_us_stocks_not_indices(self):
        ...
        for code in stocks:
            with self.subTest(code=code):
                self.assertFalse(
                    is_us_index_code(code), f"{code} should NOT be a US index"
                )
...
class TestIsUsStockCode(unittest.TestCase):
    ...
    def test_us_indices_not_stocks(self):
        ...
        for code in indices:
            with self.subTest(code=code):
                self.assertFalse(
                    is_us_stock_code(code), f"{code} should NOT be a US stock"
                )
    ...
    def test_all_indices_have_yf_symbols(self):
        ...
        for code, (symbol, name) in US_INDEX_MAPPING.items():
            with self.subTest(code=code):
                ...
                self.assertTrue(
                    symbol.startswith("^"), f"{code}'s YF symbol should start with ^"
                )
