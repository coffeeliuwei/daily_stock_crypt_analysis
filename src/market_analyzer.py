...
class MarketAnalyzer:
    ...
    def __init__(
        ...
        Args:
            ...
            region: 市场区域 cn=A股 us=美股 crypto=加密货币
        """
        self.config = get_config()
        self.search_service = search_service
        self.analyzer = analyzer
        self.data_manager = DataFetcherManager()
        self.region = region if region in ("cn", "us", "crypto") else "cn"
    ...
    def _inject_data_into_review(self, review: str, overview: MarketOverview) -> str:
        ...
        if stats_block:
            review = self._insert_after_section(
                review, r"###\s*一、市场总结", stats_block
            )

        # Inject indices table after "### 二、指数点评" section
        if indices_block:
            review = self._insert_after_section(
                review, r"###\s*二、指数点评", indices_block
            )

        # Inject sector rankings after "### 四、热点解读" section
        if sector_block:
            review = self._insert_after_section(
                review, r"###\s*四、热点解读", sector_block
            )
    ...
    def _build_indices_block(self, overview: MarketOverview) -> str:
        ...
        for idx in overview.indices:
            ...
            lines.append(
                f"| {idx.name} | {idx.current:.2f} | {arrow} {idx.change_pct:+.2f}% | {amount_str} |"
            )
        return "\n".join(lines)
    ...
    def _build_review_prompt(self, overview: MarketOverview, news: List) -> str:
        ...
        # 按 region 组装市场概况与板块区块（美股/加密货币无涨跌家数、板块数据）
        stats_block = ""
        sector_block = ""
        if self.region == "us":
            if self.profile.has_market_stats:
...
Lagging: {bottom_sectors_text if bottom_sectors_text else "N/A"}"""
            ...
            # 加密货币使用英文报告
            stats_block = "## Market Overview\n(Crypto market runs 24/7, no traditional market stats.)"
            sector_block = "## Market Sentiment\n(Fear & Greed Index and BTC dominance are key indicators.)"
        else:
            # A股
            if self.profile.has_market_stats:
                stats_block = f"""## 市场概况
...
领跌: {bottom_sectors_text if bottom_sectors_text else "暂无数据"}"""
        ...
        indices_placeholder = (
            ...
            if indices_text
                ...
                "No index data (API error)"
                if self.region in ("us", "crypto")
                else "暂无指数数据（接口异常）"
            )
        )
        news_placeholder = (
...
"""
        ...
        if self.region == "crypto":
            data_no_indices_hint_en = (
                "Note: Market data fetch failed. Rely mainly on [Market News] for qualitative analysis. Do not invent price levels."
                if not indices_text
                else ""
            )
            return f"""You are a professional cryptocurrency market analyst. Please produce a concise crypto market recap report based on the data below.
...
- No code blocks
- Use emoji sparingly in headings (at most one per heading)
- Focus on BTC and ETH as market indicators
...
## {overview.date} Crypto Market Recap

### 1. Market Summary
(2-3 sentences on overall crypto market performance, BTC/ETH moves)

### 2. Price Analysis
(Analyze BTC and ETH price action, key support/resistance levels)

### 3. Market Sentiment
(Discuss Fear & Greed Index, market情绪 if available)

### 4. Key Themes
(Analyze major catalysts, regulatory news, or market-moving events)

### 5. Outlook
(Short-term view based on technicals and news)

### 6. Risk Alerts
(Key risks to watch: regulatory, security, macro)

### 7. Strategy Plan
(Provide risk-on/neutral/risk-off stance, position sizing guideline for crypto, and key invalidation triggers.)
