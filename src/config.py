...
class Config:
    ...
    openai_model: str = "gpt-4o-mini"  # OpenAI 兼容模型名称（向后兼容，取池中第一个）
    openai_model_pool: List[str] = field(
        default_factory=list
    )  # 模型池（支持逗号分隔多个模型）
    ...
    agent_orchestrator_mode: str = (
        "standard"  # Orchestrator mode: quick/standard/full/strategy
    )
    ...
    agent_strategy_autoweight: bool = (
        True  # Auto-weight strategies by backtest performance
    )
    agent_strategy_routing: str = (
        "auto"  # Strategy routing: 'auto' (regime-based) or 'manual'
    )
    ...
    max_stocks_per_worker: int = 10  # 每个并发最大处理股票数（动态并发计算）
    debug: bool = False
    http_proxy: Optional[str] = None  # HTTP 代理 (例如: http://127.0.0.1:10809)
    https_proxy: Optional[str] = None  # HTTPS 代理

    # === 数据源池配置 ===
    # 数据源选择模式：random(加权随机) / priority(优先级顺序) / round_robin(轮询)
    source_selection_mode: str = "random"
    # 连续失败次数触发冷却
    ...
    # === LLM 模型池配置 ===
    # 模型选择模式：random(随机) / sequential(顺序) / least_latency(最低延迟)
    model_selection_mode: str = "random"
    ...
    def __post_init__(self) -> None:
        ...
        if self.agent_strategy_routing not in self._VALID_STRATEGY_ROUTING:
            _log.warning(
                ...
                self.agent_strategy_routing,
                self._VALID_STRATEGY_ROUTING,
            )
    ...
    def _load_from_env(cls) -> "Config":
        ...
        if not llm_model_list:
            llm_model_list = cls._legacy_keys_to_model_list(
                ...
                or ("https://aihubmix.com/v1" if os.getenv("AIHUBMIX_KEY") else None),
                deepseek_api_keys,
            )
            if llm_model_list:
                llm_models_source = "legacy_env"
        ...
        return cls(
            ...
            openai_api_key=os.getenv("AIHUBMIX_KEY")
            or os.getenv("OPENAI_API_KEY")
            or None,
            openai_base_url=os.getenv("OPENAI_BASE_URL")
            or ("https://aihubmix.com/v1" if os.getenv("AIHUBMIX_KEY") else None),  # noqa: E501
            openai_model=cls._parse_openai_model(
                os.getenv("OPENAI_MODEL", "gpt-4o-mini")
            )[0],
            openai_model_pool=cls._parse_openai_model(
                os.getenv("OPENAI_MODEL", "gpt-4o-mini")
            ),
            ...
            agent_orchestrator_timeout_s=max(
                0, int(os.getenv("AGENT_ORCHESTRATOR_TIMEOUT_S", "600"))
            ),
            agent_risk_override=os.getenv("AGENT_RISK_OVERRIDE", "true").lower()
            == "true",
            agent_deep_research_budget=int(
                os.getenv("AGENT_DEEP_RESEARCH_BUDGET", "30000")
            ),
            agent_deep_research_timeout=max(
                30, int(os.getenv("AGENT_DEEP_RESEARCH_TIMEOUT", "180"))
            ),
            agent_memory_enabled=os.getenv("AGENT_MEMORY_ENABLED", "false").lower()
            == "true",
            agent_strategy_autoweight=os.getenv(
            ...
            == "true",
            agent_event_monitor_interval_minutes=max(
                1, int(os.getenv("AGENT_EVENT_MONITOR_INTERVAL_MINUTES", "5"))
            ),
            ...
            markdown_to_image_max_chars=int(
                os.getenv("MARKDOWN_TO_IMAGE_MAX_CHARS", "15000")
            ),
            ...
            max_stocks_per_worker=int(os.getenv("MAX_STOCKS_PER_WORKER", "10")),
            debug=os.getenv("DEBUG", "false").lower() == "true",
            config_validate_mode=os.getenv("CONFIG_VALIDATE_MODE", "warn").lower(),
            http_proxy=os.getenv("HTTP_PROXY"),
            https_proxy=os.getenv("HTTPS_PROXY"),
            # 数据源池配置
            source_selection_mode=os.getenv("SOURCE_SELECTION_MODE", "random").lower(),
            source_failure_threshold=int(os.getenv("SOURCE_FAILURE_THRESHOLD", "3")),
            source_cooldown_seconds=int(os.getenv("SOURCE_COOLDOWN_SECONDS", "300")),
            # 模型池配置
            model_selection_mode=os.getenv("MODEL_SELECTION_MODE", "random").lower(),
            ...
            fundamental_cache_ttl_seconds=int(
                os.getenv("FUNDAMENTAL_CACHE_TTL_SECONDS", "120")
            ),
    ...
    @classmethod
    def _parse_openai_model(cls, model_str: str) -> List[str]:
        """
        解析 OPENAI_MODEL 环境变量，支持逗号分隔的多个模型。

        Args:
            model_str: OPENAI_MODEL 环境变量的值，如 "gpt-4o-mini" 或 "qwen-turbo,qwen-plus,qwen-max"

        Returns:
            模型列表，如 ["gpt-4o-mini"] 或 ["qwen-turbo", "qwen-plus", "qwen-max"]

        示例：
            - "gpt-4o-mini" → ["gpt-4o-mini"]
            - "qwen-turbo,qwen-plus,qwen-max" → ["qwen-turbo", "qwen-plus", "qwen-max"]
            - "" → ["gpt-4o-mini"] (默认值)
        """
