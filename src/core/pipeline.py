...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            if not force_refresh and self.db.has_today_data(code, today):
                logger.info(
                    f"{stock_name}({code}) 今日数据已存在，跳过获取（断点续传）"
                )
                return True, None
    ...
    def analyze_stock(
        self, code: str, report_type: ReportType, query_id: str
        ...
        if _is_crypto_code(code):
            return self._analyze_crypto(code, report_type, query_id)
        ...
        try:
            ...
            try:
                ...
                if realtime_quote:
                    ...
                    logger.info(
                        f"{stock_name}({code}) 实时行情获取失败或已禁用，将使用历史数据进行分析"
                    )
            except Exception as e:
            ...
            try:
                fundamental_context = self.fetcher_manager.get_fundamental_context(
                    ...
                    budget_seconds=getattr(
                        self.config, "fundamental_stage_timeout_seconds", 1.5
                    ),
                )
            except Exception as e:
    ...
    def _analyze_crypto(
        ...
        流程：
        1. 获取实时行情（通过 CryptoFetcher → QVeris/CCXT）
        2. 获取历史数据和技术指标
        3. 获取恐惧贪婪指数
        4. 获取加密货币新闻（通过 QVeris）
        5. 调用 AI 进行综合分析

        Args:
            code: 加密货币代码（如 BTC, ETH）
            report_type: 报告类型
            query_id: 查询 ID
        ...
        from src.services.crypto_sentiment_service import CryptoSentimentService
        from src.services.crypto_news_fetcher import CryptoNewsFetcher

        # 获取加密货币名称
        base_symbol = (
            code.upper().replace("USDT", "").replace("USD", "").replace("-USD", "")
        )
        crypto_name = CRYPTO_NAMES.get(base_symbol, base_symbol)

        logger.info(f"[CryptoPipeline] 开始分析 {crypto_name}({code})")
        ...
        try:
            ...
            try:
                ...
                if realtime_quote:
                    logger.info(
                        f"{crypto_name}({code}) 实时行情: 价格={realtime_quote.price}"
                    )
            except Exception as e:
                logger.warning(f"{crypto_name}({code}) 获取实时行情失败: {e}")
            ...
            try:
                ...
                if daily_df is not None and not daily_df.empty:
                    logger.info(
                        f"{crypto_name}({code}) 历史数据获取成功: {len(daily_df)} 条 (来源: {source_name})"
                    )
            except Exception as e:
            ...
            if daily_df is not None and not daily_df.empty:
                try:
                    ...
                    logger.warning(f"{crypto_name}({code}) 趋势分析失败: {e}")
            ...
            try:
                ...
                if fear_greed_data:
                    logger.info(
                        f"{crypto_name} 恐惧贪婪指数: {fear_greed_data.get('value')} ({fear_greed_data.get('classification')})"
                    )
            except Exception as e:
                logger.warning(f"{crypto_name} 获取恐惧贪婪指数失败: {e}")

            # Step 5: 获取加密货币新闻
            news_context = None
            try:
                news_fetcher = CryptoNewsFetcher()
                news_context = news_fetcher.fetch_crypto_news(code, crypto_name)
                if news_context:
                    logger.info(f"{crypto_name}({code}) 新闻获取成功")
            except Exception as e:
                logger.warning(f"{crypto_name}({code}) 获取新闻失败: {e}")
            ...
            crypto_context = {
                ...
                "realtime": {
                    "name": crypto_name,
                    "price": realtime_quote.price if realtime_quote else None,
                    "change_pct": realtime_quote.change_pct if realtime_quote else None,
                    "volume": realtime_quote.volume if realtime_quote else None,
                    "market_cap": getattr(realtime_quote, "total_mv", None)
                    if realtime_quote
                    else None,
                },
            ...
            result = self.analyzer.analyze_crypto(
                crypto_context,
                news_context=news_context,
            ...
            if result:
                result.query_id = query_id
                result.current_price = crypto_context["realtime"]["price"]
                result.change_pct = crypto_context["realtime"]["change_pct"]
            ...
            if result:
                try:
                    ...
                    logger.warning(f"{crypto_name}({code}) 保存分析历史失败: {e}")

            return result

        except Exception as e:
            logger.error(f"{crypto_name}({code}) 加密货币分析失败: {e}")
            logger.exception(f"{crypto_name}({code}) 详细错误信息:")
            return None
    ...
    def run(
        ...
        if single_stock_notify:
            logger.info(
                f"已启用单股推送模式：每分析完一只股票立即推送（报告类型: {report_type_str}）"
            )
        ...
        try:
            ...
            if self.notifier.is_available():
                ...
                if NotificationChannel.WECHAT in channels:
                    ...
                    if use_image:
                        wechat_success = self.notifier._send_wechat_image(
                            wechat_image_bytes
                        )
                    else:
