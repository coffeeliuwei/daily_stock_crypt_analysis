...
class BaseSearchProvider(ABC):
    ...
    def _record_error(self, key: str) -> None:
        ...
        logger.warning(
            f"[{self._name}] API Key {key[:8]}... 错误计数: {self._key_errors[key]}"
        )
...
class TavilySearchProvider(BaseSearchProvider):
    ...
    def _do_search(
        ...
        try:
            ...
            # 记录原始响应到日志
            logger.info(
                f"[Tavily] 搜索完成，query='{query}', 返回 {len(response.get('results', []))} 条结果"
            )
            ...
            for item in value_list[:max_results]:
                ...
                results.append(
                    SearchResult(
                        ...
                        or self._extract_domain(item.get("url", "")),
                ...
                if raw_published_date:
                    try:
                        dt = datetime.fromisoformat(
                            raw_published_date.replace("Z", "+00:00")
                        )
                        published_date = dt.strftime("%Y-%m-%d")
                    except (ValueError, AttributeError):
...
class GNewsSearchProvider(BaseSearchProvider):
    """
    GNews 搜索引擎 - 免费 Google News 抓取

    特点：
    - 完全免费，无需 API Key
    - 支持多语言和多国家
    - 支持 Google News 头条和关键词搜索

    文档：https://github.com/ranahaani/GNews
    """

    def __init__(self):
        # GNews 不需要 API Key，但 BaseSearchProvider 需要 keys 列表
        super().__init__(["free"], "GNews")
    ...
    def _do_search(
        ...
        try:
            ...
            return SearchResponse(
                ...
                provider=self.name,
                success=False,
                error_message="gnews 未安装，请运行: pip install gnews",
            )
        ...
        try:
            ...
            error_msg = str(e)
            logger.error(f"[GNews] 搜索失败: {error_msg}")
            return SearchResponse(
                query=query,
                results=[],
                provider=self.name,
...
class FinnhubSearchProvider(BaseSearchProvider):
    ...
    def _do_search(
        ...
        try:
            ...
            # 首先尝试获取市场新闻 (general news)
            params = {
                "category": "general",
            }

            response = _get_with_retry(
                f"{self.API_BASE_URL}/news", headers=headers, params=params, timeout=15
            )
            ...
            for item in data[: max_results * 2]:  # 多取一些然后过滤
...
class MarketAuxSearchProvider(BaseSearchProvider):
    """
    MarketAux 金融新闻搜索
    ...
    文档：https://www.marketaux.com/documentation
    """

    API_BASE_URL = "https://api.marketaux.com/v1"

    def __init__(self, api_keys: List[str]):
        super().__init__(api_keys, "MarketAux")
    ...
    def _do_search(
        ...
        try:
            ...
            response = _get_with_retry(
                f"{self.API_BASE_URL}/news/all", headers={}, params=params, timeout=15
            )
...
class FreeNewsAPISearchProvider(BaseSearchProvider):
    """
    FreeNewsAPI 搜索引擎
    ...
    文档：https://www.freenewsapi.com/
    """

    API_BASE_URL = "https://www.freenewsapi.com"

    def __init__(self):
        # 不需要 API Key
        super().__init__(["free"], "FreeNewsAPI")
    ...
    def _do_search(
        ...
        """执行 FreeNewsAPI 搜索"""
        try:
            # 按关键词搜索 - FreeNewsAPI 不需要 headers 和 params
            response = requests.get(f"{self.API_BASE_URL}/k/{query}", timeout=15)
            ...
            for item in articles[:max_results]:
                ...
                results.append(
                    SearchResult(
                        ...
                        url=url,
                        source=self._extract_domain(url) if url else "FreeNewsAPI",
                        published_date=None,  # FreeNewsAPI 返回的是 sec 时间戳
                    )
                )
...
class SearchService:
    ...
    def __init__(
        ...
        marketaux_keys: Optional[List[str]] = None,
        enable_gnews: bool = True,
        enable_freenews: bool = True,
        ...
        Args:
            ...
            finnhub_keys: Finnhub API Key 列表（金融专用新闻）
            marketaux_keys: MarketAux API Key 列表（金融新闻+实体追踪）
            enable_gnews: 是否启用 GNews（免费，无需 API Key）
            enable_freenews: 是否启用 FreeNewsAPI（免费，无需 API Key）
        ...
        self.news_strategy_profile = normalize_news_strategy_profile(
        ...
        # 1. GNews（完全免费，Google News 抓取，无配额限制）
        if enable_gnews:
            self._providers.append(GNewsSearchProvider())
            logger.info("已配置 GNews 搜索（免费，优先级最高）")

        # 2. FreeNewsAPI（完全免费，100万次/天）
        if enable_freenews:
            self._providers.append(FreeNewsAPISearchProvider())
            logger.info("已配置 FreeNewsAPI 搜索（免费）")
        ...
        # 8. Finnhub（金融专用新闻，免费版 60次/分钟）
        if finnhub_keys:
            self._providers.append(FinnhubSearchProvider(finnhub_keys))
            logger.info(f"已配置 Finnhub 搜索，共 {len(finnhub_keys)} 个 API Key")

        # 9. MarketAux（金融新闻+实体追踪，免费版 100次/天）
        if marketaux_keys:
            self._providers.append(MarketAuxSearchProvider(marketaux_keys))
            logger.info(f"已配置 MarketAux 搜索，共 {len(marketaux_keys)} 个 API Key")

        # 10. SearXNG（自建实例，无配额，最后兜底）
    ...
    def is_index_or_etf(stock_code: str, stock_name: str) -> bool:
        ...
        if (
            code.isdigit()
            and len(code) == 6
            and code.startswith(SearchService._A_ETF_PREFIXES)
        ):
            return True
    ...
    def _provider_request_size(cls, max_results: int) -> int:
        ...
        return max(
            target, min(target * cls.NEWS_OVERSAMPLE_FACTOR, cls.NEWS_OVERSAMPLE_MAX)
        )
    ...
    def search_stock_price_fallback(
        ...
        is_foreign = self._is_foreign_stock(stock_code)
        keywords = (
            self.ENHANCED_SEARCH_KEYWORDS_EN
            if is_foreign
            else self.ENHANCED_SEARCH_KEYWORDS
        )
            ...
            for provider in self._providers:
                ...
                try:
                    ...
                    if response.success and response.results:
                        ...
                        logger.info(
                            f"[增强搜索] {provider.name} 返回 {len(response.results)} 条结果"
                        )
                        break  # 成功后跳到下一个关键词
                    else:
        ...
        if all_results:
            ...
            provider_str = (
                ", ".join(successful_providers) if successful_providers else "None"
            )

            logger.info(
                f"[增强搜索] 完成，共获取 {len(final_results)} 条结果（来源: {provider_str}）"
            )
    ...
    def search_stock_with_enhanced_fallback(
        ...
        if include_news:
            results["news"] = self.search_stock_news(
                stock_code, stock_name, max_results=max_results
            )
...
def get_search_service() -> SearchService:
    ...
    if _search_service is None:
        ...
        _search_service = SearchService(
            ...
            finnhub_keys=getattr(config, "finnhub_api_keys", []),
            marketaux_keys=getattr(config, "marketaux_api_keys", []),
            enable_gnews=getattr(config, "enable_gnews", True),
            enable_freenews=getattr(config, "enable_freenews", True),
