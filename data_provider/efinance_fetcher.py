...
def _ef_call_with_timeout(
    func: Any, *args: Any, timeout: Optional[float] = None, **kwargs: Any
) -> Any:
...
class EfinanceFetcher(BaseFetcher):
    ...
    def _fetch_etf_data(
        ...
        try:
            ...
            logger.error(failure_message)
            raise DataFetchError(
                f"efinance 获取 ETF 数据失败: {failure_message}"
            ) from e
        ...
        try:
            ...
            if (
                ...
                if df is not None and not df.empty:
                    ...
                    logger.warning(
                        f"[API返回] ETF 实时行情为空, 耗时 {api_elapsed:.2f}s"
                    )
                    df = pd.DataFrame()
            ...
            if row.empty:
                logger.warning(
                    f"[API返回] 未找到 ETF {stock_code} 的实时行情(efinance)"
                )
                return None
