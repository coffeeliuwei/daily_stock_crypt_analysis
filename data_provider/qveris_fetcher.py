...
优先级策略：
- 加密货币: Priority 0 (主数据源)
- 股票: Priority 2 (等同于 Tushare)
...
class QVerisFetcher(BaseFetcher):
    ...
    动态优先级策略：
    - 加密货币: Priority 0 (主数据源，最高优先)
    - 股票: Priority 2 (等同于 Tushare，在免费数据源之后)
    ...
    name = "QVerisFetcher"
    priority = 2  # 默认优先级（股票），等同于 Tushare

    # 加密货币优先级
    CRYPTO_PRIORITY = 0  # 加密货币主数据源
    STOCK_PRIORITY = 2  # 股票备用数据源
    ...
    def __init__(self):
        ...
        if not self.api_key:
            ...
            logger.info(
                "[QVerisFetcher] 已配置 API Key - 加密货币(主数据源), 股票(备用,等同于Tushare)"
            )
    ...
    def get_priority_for_code(self, stock_code: str) -> int:
        ...
        Returns:
            优先级数字（越小越优先）
        """
        from .base import _is_crypto_code

        if _is_crypto_code(stock_code):
            return self.CRYPTO_PRIORITY
        return self.STOCK_PRIORITY
    ...
    def _fetch_raw_data(
        ...
        search_result = self._search_tool(
            "stock historical data OHLC K-line candlestick", limit=5
        )
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        try:
            # 搜索实时行情工具
            search_result = self._search_tool(
                "real-time stock price quote current", limit=5
            )
