...
class DataFetcherManager:
    ...
    def _is_missing_board_value(value: Any) -> bool:
        ...
        try:
            ...
            # pd.isna() may raise TypeError for non-hashable types
            logger.debug(
                f"[DataFetcherManager] pd.isna() check failed for value type {type(value)}"
            )
    ...
    def _init_default_fetchers(self) -> None:
        ...
        # 初始化数据源列表（按优先级排序）
        # 注意：QVeris 对加密货币有动态优先级（Priority 0），对股票为 Priority 2
        self._fetchers = [
    ...
    def get_realtime_quote(self, stock_code: str) -> Optional["UnifiedRealtimeQuote"]:
                    ...
                    if hasattr(fetcher, "get_realtime_quote"):
                        try:
                            ...
                            if quote is not None:
                                logger.info(
                                    f"[实时行情] 美股指数 {stock_code} 成功获取 (来源: yfinance)"
                                )
                                return quote
                        except Exception as e:
                            logger.warning(
                                f"[实时行情] 美股指数 {stock_code} 获取失败: {e}"
                            )
                    break
                ...
                if fetcher.name == "YfinanceFetcher":
                    if hasattr(fetcher, "get_realtime_quote"):
                        try:
                            ...
                            if quote is not None:
                                logger.info(
                                    f"[实时行情] 美股 {stock_code} 成功获取 (来源: yfinance)"
                                )
                                return quote
                        except Exception as e:
                            logger.warning(
                                f"[实时行情] 美股 {stock_code} 获取失败: {e}"
                            )
                    break
            ...
            for fetcher in self._fetchers:
                ...
                try:
                    ...
                    if quote is not None and quote.has_basic_data():
                        logger.info(
                            f"[实时行情] 港股 {stock_code} 成功获取 (来源: akshare_hk)"
                        )
                        return quote
                except Exception as e:
        ...
        if errors:
            logger.warning(
                f"[实时行情] {stock_code} 所有数据源均失败，降级处理: {'; '.join(errors)}"
            )
        else:
    ...
    def _get_fundamental_config(self) -> "Config":
        from src.config import get_config
    ...
    def _has_meaningful_payload(payload: Any) -> bool:
        ...
        try:
            ...
            # pd.isna() may raise TypeError for non-hashable types
            logger.debug(
                f"[DataFetcherManager] pd.isna() check failed for payload type {type(payload)}"
            )
        return True
